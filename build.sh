...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e sourcesocket.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/sourcesocket-1.0.1
        tar czf ${tmpdir}/sourcesocket-1.0.1.tar.gz --exclude=".svn" -C ${tmpdir} sourcesocket-1.0.1
        rpmbuild -ta ${tmpdir}/sourcesocket-1.0.1.tar.gz
        rm -rf $tmpdir
    else
