...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.sourcesocket.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.sourcesocket-1.0.2
        tar czf ${tmpdir}/rh.sourcesocket-1.0.2.tar.gz --exclude=".svn" -C ${tmpdir} rh.sourcesocket-1.0.2
        rpmbuild -ta ${tmpdir}/rh.sourcesocket-1.0.2.tar.gz
        rm -rf $tmpdir
    else
